...
import net.dirtcraft.plugins.dirtlimits.utils.gradient.GradientHandler;
...
public class ListCommand {
	...
	private static void showLimitsList(CommandSender sender, String playerName, int page) {
		...
		if (page == maxPage) {
			...
			if (senderEqualsPlayer) {
				pageNext.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/dirtlimit list " + (page + 1)));
			} else {
				pageNext.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/dirtlimit list " + sender.getName() + " " + (page + 1) ));
			}
		}
